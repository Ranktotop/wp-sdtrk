...
//				$this->config = wp_parse_args( $this->config, $default );
		...
		public function get_value( $options, $field  ) {
			 ...
			 *
			 *	   if ( $unique == $this->wp_sls && $field['id'] == 'your-field-id' ) {
			 *		   // do the magic ;)
			 *	   }
			 *
			 *	   return $value;
...
/**
 * Use:
 *  $options = ( function_exists( 'get_exopite_sof_option' ) ) ? get_exopite_sof_option( $this->wp_sls ) : get_option( $this->wp_sls );

 *  // OR
 *
 *  if ( function_exists( 'get_exopite_sof_option' ) ) {
 *      $options = get_exopite_sof_option( $this->wp_sls );
 *  } else {
 *      $options = get_option( $this->wp_sls );
 *  }
 */
if ( ! function_exists( 'get_exopite_sof_option' ) ) {
