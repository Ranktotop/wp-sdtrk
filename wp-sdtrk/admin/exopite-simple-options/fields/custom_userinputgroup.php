...
if ( ! class_exists( 'Wp_Sdtrk_Exopite_Simple_Options_Framework_Field_custom_userinputgroup' ) ) {
    class Wp_Sdtrk_Exopite_Simple_Options_Framework_Field_custom_userinputgroup extends Wp_Sdtrk_Exopite_Simple_Options_Framework_Fields {
		...
		public function __construct( $field, $value = '', $unique = '', $config = array() ) {
			...
			$this->license = ( isset( $this->field['options']['license'] ) ) ? $this->field['options']['license'] : $defaults['license'];

			if ( ! $this->is_repeater ) {
				$this->is_cloneable = false;
				$this->is_sortable  = false;
			} else {
			    //If activation isnt needed or is true
			    if($this->license === false || Wp_Sdtrk_License::wp_sdtrk_getLicenseData($this->license)["isValid"] === true){
				...
				if ( $this->value ) {
					...
					foreach ( $this->value as $key => $value ) {
						...
						foreach ( $fields as $field ) {
						   ...
						   $field = $this->disableFields($field);
		...
		/**
		 * Disable fields if license is set
		 * @param string[] $field
		 * @return string[]
		 */
		private function disableFields($field){
